  ...
  getDataToken()
    .then(datos => {
      ...
      .then(res => {
        ...
        .catch(error => {
        console.log("Error consultando las apis de configuracion general",error.toString());
        });
...
const queryGeneralFunction = (datos, execute) => {
  axios.get(queryGeneral, datos)
    .then(res => {
      execute(res.data);
    })
    .catch(error => {
      console.log("Error consultando la api de query general", error.toString());
    });
};
