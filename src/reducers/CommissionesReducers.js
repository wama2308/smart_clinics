...
const LoadConfigCommissionIdFunction = (state, payload) => {
	...
	estado.servicesAllCommission = payload.data.services_commissions;
	estado.servicesPayment = payload.data.services_payments;
	estado.servicesAllPayment = payload.data.services_payments;
...
const setPorcentajeTableComisiones = (state, payload) => {
	...
	const key_ = estado.servicesAllPayment.findIndex(service => service.serviceId === payload.id);
	estado.servicesPayment[key].percentage = payload.value;
	estado.servicesAllPayment[key_].percentage = payload.value;
	estado.action = 1;
	return Map(estado);
}
...
const setSwitchTableComisiones = (state, payload) => {
	...
	if (payload.tab === "2" && payload.typePersonal !== "5d1776e3b0d4a50b23936710") {
		...
		const key_ = estado.servicesAllCommission.findIndex(service => service.serviceId === payload.id);
		estado.servicesCommission[key].confirm = payload.value;
		estado.servicesAllCommission[key_].confirm = payload.value;
	} else if (payload.tab === "3") {
		const key = estado.servicesPayment.findIndex(service => service.serviceId === payload.id);
		const key_ = estado.servicesAllPayment.findIndex(service => service.serviceId === payload.id);
		estado.servicesPayment[key].confirm = payload.value;
		estado.servicesAllPayment[key_].confirm = payload.value;
	}
	estado.action = 1;
	return Map(estado);
}
const setCleanListServices = (state, payload) => {
	...
	estado.servicesAllCommission.map((service, i) => {
		service.percentage = payload.percentaje;
		service.confirm = payload.confirm;
	})
	...
	estado.servicesAllPayment.map((service, i) => {
		service.percentage = payload.percentaje;
		service.confirm = payload.confirm;
	})
...
const setSwitchAllTableComisiones = (state, payload) => {
	...
	if (payload.tab === "2" && payload.typePersonal !== "5d1776e3b0d4a50b23936710") {
		...
		estado.servicesAllCommission.map((commission, i) => {
			commission.confirm = payload.value;
			estado.action = 1;
		})
	} else if (payload.tab === "3") {
		...
		estado.servicesAllPayment.map((payment, i) => {
			payment.confirm = payload.value;
			estado.action = 1;
		})
	}
...
const filterServicesAction = (state, payload) => {
	let estado = state.toJS();
	if (payload.tab === "2") {
		const dataService = getArray(estado.servicesAllCommission)
		let expresion = new RegExp(`${payload.value}.*`, "i");
		const result = dataService.filter(data => expresion.test(data.serviceName));		
		estado.servicesCommission = result;
	} else {
		const dataService = getArray(estado.servicesAllPayment)
		let expresion = new RegExp(`${payload.value}.*`, "i");
		const result = dataService.filter(data => expresion.test(data.serviceName));
		estado.servicesPayment = result;
	}
	return Map(estado);
}
...
const ConfigCommissionsReducer = (state = Map(), action) => {
	switch (action.type) {
		...
		case "SET_FILTER_SERVICES":
			return filterServicesAction(state, action.payload);
